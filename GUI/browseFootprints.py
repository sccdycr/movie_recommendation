...
class BrowseFootprints():
    ...
    def __init__(self,root,window,movieid,userid,):
        ...
        GUI.metaButtons.NavigationBar(self.root, self.window, navbar_Frame, self.movieid, self.userid, type="B")

        #从mysql中提取用户的浏览记录
        conn,cur = GlobalFun.ConnectSql()
        sql = "select movieid,rating,timestamp from dbmovierecommender.ratings where userid={}  order by timestamp desc limit 15;".format(self.userid)
        cur.execute(sql)
        data = cur.fetchall()#记录用户的打分情况
        sql = "select count(rating) from dbmovierecommender.ratings where userid={};".format(self.userid)
        cur.execute(sql)
    ...
    def job(self,Frame,movieid,rating,timestamp):
        temp = GUI.metaFrame.metaFrame(movieid, self.userid, Frame, self.window, self.root,type='browse',timestamp=timestamp,rate=rating)
