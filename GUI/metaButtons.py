...
class Login_register_Button():
    ...
    def comfirm(self):
        ...
        cur.execute("select password from dbmovierecommender.users where userid={}".format(self.name.get()))
        data = cur.fetchall()
        if len(data)==0:
            #记录新用户
            # cur.execute("insert into dbmovierecommender.users values({},{})".format(self.name.get(),self.password.get()))
            cur.execute(
                "insert into dbmovierecommender.users set userid={},password={},usercount=0;".format(self.name.get(), self.password.get()))
            conn.commit()
    ...
    def update(self):
        ...
        if self.type == "M" or self.type == "B":
            GUI.main_window.Main_window(self.root, self.newFrame, self.movieid, self.userid)
        elif self.type == "I":
            GUI.movieInfo_window.movieinfo_window(self.root, self.newFrame, self.movieid, self.userid)
        else:
            GUI.search_window.Search_window(self.root,self.newFrame,self.movieid,self.userid)
...
class Browse_Button():
    ...
    def turn2Browse(self):
        ...
        GUI.browseFootprints.BrowseFootprints(self.root, self.newFrame, self.movieid, self.userid)
        print('Well Done!')
